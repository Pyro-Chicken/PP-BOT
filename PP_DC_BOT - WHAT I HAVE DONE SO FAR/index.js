...
bot.on('ready', async () => {
	...
	bot.user.setActivity(`Ping Me! (@PP Bot)`, {type: "LISTENING"}); 
...
//Hello Command. If command === hello, then it will reply with "Hi!". This can be used to test if Discord Bot is working (like a test command).
...
bot.on("messageUpdate", function(oldMessage, newMessage) {
	...
	const LogChannel = newMessage.guild.channels.cache.find(c => c.name === 'logs');


	if(!LogChannel){
		channel.send('Someone has updated a message, but I am unable to log it as there is no Logs channel. In order to recieve logs please make a channel named "logs" (without the quotation marks)')
		return;
	}
...
bot.on("guildMemberRemove", member => {
	...
	const LogChannel = member.guild.channels.cache.find(c => c.name === 'join-leave')
	const channel = member.guild.channels.cache.find((c) => c.type === 'text')


	if(!LogChannel){
		channel.send('Someone has left, but I am unable to log it as there is no join-leave channel. In order to recieve logs please make a channel named "join-leave" (without the quotation marks)')
		return
	}
...
bot.on("guildMemberAdd", member => {
	...
	if(!LogChannel){
		channel.send('Someone has joined, but I am unable to log it as there is no join-leave channel. In order to recieve logs please make a channel named "join-leave" (without the quotation marks)')
		return
	}
...
if(channel.type ==='dm' ){
	...
	if(!LogChannel){
		text.send('A channel has been created, but I am unable to log it as there is no logs channel. In order to recieve logs please make a channel named "logs" (without the quotation marks)')
		return
	}
...
bot.on("channelDelete", channel => {
	...
	if(!LogChannel){
		text.send('A channel has been deleted, but I am unable to log it as there is no logs channel. In order to recieve logs please make a channel named "logs" (without the quotation marks)')
		return
	}
